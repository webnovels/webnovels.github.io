...
async function processNovel() {
  try {
    ...
    try {
      ...
      console.log('_novels directory might not exist yet, will create it');
    }
    ...
    for (let i = 3; i < lines.length; i++) {
      ...
      if (/^[0-9]+$/.test(line)||/^Chapter\s+\d+$/i.test(line)) {
        isContent = false;
...
next_chapter: "${nextChapter?nextChapter:`/novles/${novelId}/more/`}"
